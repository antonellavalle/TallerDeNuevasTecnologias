...
export function FilterModal({
    ...
    useEffect(() => setTipos(selectedTipos), [selectedTipos]);
    useEffect(() => setGeneros(selectedGeneros), [selectedGeneros]);

    const toggleTipo = (id: number) =>
        setTipos((prev) => (prev.includes(id) ? prev.filter((t) => t !== id) : [...prev, id]));

    const toggleGenero = (id: number) =>
        setGeneros((prev) => (prev.includes(id) ? prev.filter((g) => g !== id) : [...prev, id]));
    ...
    return (
        ...
        <View style={{ flex: 1 }}>
            ...
            {tiposApi.map((tipo) => (
                ...
                <CustomCheckbox checked={tipos.includes(tipo.id)} onPress={() => toggleTipo(tipo.id)} />
            ...
            <View style={styles.checkboxGrid}>
                ...
                <View key={genero.id} style={styles.checkboxColumn}>
                    ...
                    <CustomCheckbox checked={generos.includes(genero.id)} onPress={() => toggleGenero(genero.id)} />
